...
import { TargetIcon, SparklesIcon, CreditCardIcon, CogIcon, LogoutIcon, BoltIcon } from '../Icons';
...
const ClientLayout: React.FC<ClientLayoutProps> = ({ user, onLogout, refreshProfile }) => {
  ...
  const navItems = [
    { label: 'Leads Dashboard', path: '/portal', icon: <TargetIcon className="w-5 h-5" /> },
      ...
      <aside className="w-72 hidden lg:flex flex-col bg-white border-r border-slate-200 fixed inset-y-0 shadow-sm">
        <div className="h-16 flex items-center px-8 border-b border-slate-100">
          <Link to="/" className="flex items-center space-x-2">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900 font-heading">AuraFunnel</span>
          </Link>
        </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/50">
          <div className="p-5 bg-slate-900 rounded-3xl text-white shadow-2xl">
            ...
            <div className="w-full bg-slate-800 h-1 rounded-full overflow-hidden mb-3">
              <div className="bg-indigo-500 h-full rounded-full transition-all duration-1000" style={{ width: `${usagePercentage}%` }}></div>
            </div>
