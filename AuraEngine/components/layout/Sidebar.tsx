        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center rounded-xl transition-all duration-150 ease-out group relative ${collapsed ? 'justify-center px-3 py-3' : 'px-3 py-2.5 gap-3'} ${isActive
                ? isLight
            ...
            >
              ...
              <span className={`shrink-0 [&>svg]:w-5 [&>svg]:h-5 transition-colors duration-150 ${isActive ? '' : isLight ? 'text-gray-400 group-hover:text-gray-600' : 'text-slate-500 group-hover:text-slate-300'}`}>
                {item.icon}
              </span>
