...
### BUG-018 — Real lead intelligence hidden behind default-off flag — ✅ FIXED (Phase 4.D + Roadmap 1.2)
...
- ✅ **Phase 4.D:** the arbitrary "+5 per click" score bump is gone. The LeadProfile "Recalculate Score" button calls the canonical scorer `lib/leadScoring.ts` `recalcLeadScore` — deterministic formula over firmographics, email engagement, Mails.so deliverability, recency and suppression — persisting the breakdown to `lead_scores` and syncing `leads.score`.
- ✅ **Roadmap 1.2:** `lead_intelligence` is now **ON by default** (`isFlagEnabledDefaultOn`; a `workspace_feature_flags` row with enabled=false opts out). Safe — the Score/Research/Next-action panels only READ cached data on mount; AI generation stays behind explicit credit-gated buttons. Added `recalcLeadScoresBulk` (batched gather + bulk `lead_scores` upsert + `leads.score` sync, no AI/credits) and a **"Score" action** on the Leads page that recomputes every lead in the business (with progress) so the table/kanban show real scores instead of 0. ✅ **Follow-ons also done:** `getLeadScoresBulk` powers a **reason tooltip** on the score cell (table + kanban: `{total}/100 — {reason_summary} (confidence X%)`), and imports **auto-score** the new batch (`ImportLeadsWizard` → `recalcLeadScoresBulk` by `import_batch_id`) so freshly imported leads never show 0.
