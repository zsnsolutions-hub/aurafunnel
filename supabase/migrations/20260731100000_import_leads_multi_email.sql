-- ============================================================================
-- Multi-email import. Previously only one address (primary_email) was kept and
-- extra email columns collided/were dropped; leads.emails[] was never filled.
--
-- Now the RPC collects emails from the primary_email column AND every
-- additional_emails column, splitting multi-address cells on [ ; , | / whitespace ].
-- primary_email = the first address of the primary_email column (first-mapped
-- wins); leads.emails[] = the full deduped set (primary first). Merge/overwrite
-- union the arrays. Rest of the function is the current definition, verbatim.
-- ============================================================================
...
begin
  ...
  for v_row in select * from jsonb_array_elements(p_rows) loop
    ...
    v_insights := null; v_custom := '{}'::jsonb; v_emails := '{}'::text[];
    ...
    insert into public.leads (
      client_id, workspace_id, business_id, company, score, status, source, insights,
      ...
      p_workspace_id, p_workspace_id, v_business_id, v_company,
