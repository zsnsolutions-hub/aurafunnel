-- ============================================================================
-- Fix the same NOT-NULL workspace_id bug (see notnull-workspace-id-rpc-bug) in
-- the admin_* RPCs: their audit_logs insert omitted workspace_id, so every
-- admin action (grant/adjust credits, change/clone/update plan, reset usage,
-- update entitlements/flags) failed with a not-null violation. Each insert now
-- sets workspace_id = the actor id (p_admin_id), matching the ws==user_id
-- convention. Definitions are otherwise verbatim from the live DB.
-- ============================================================================
