-- ============================================================================
-- Fix: CSV/XLSX import was fully broken. import_leads_batch inserted into leads
-- WITHOUT workspace_id (NOT NULL, no default) → every import failed on row 1
-- with a not-null violation. Also never set business_id, so even a fixed import
-- would land leads invisible under the active-business filter.
--
-- This recreates the function with two changes only:
--   • declare + read v_business_id from p_options->>'business_id'
--   • the leads INSERT now sets workspace_id (= p_workspace_id) and business_id.
-- Everything else is the live definition, verbatim.
-- ============================================================================
